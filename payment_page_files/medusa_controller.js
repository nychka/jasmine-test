$.Controller("MedusaController",{
  ...
  *
  */
  initializePaymentCard: function(){
    var card_types = $.archive && $.archive.getData().configs && $.archive.getData().configs.additional_card_types;


    if(card_types && Object.keys(card_types).length) {
  ...
  */
  ...
  "[name=use_usblp_bonuses] -> change": function(ev){ // USBLP CHECKBOX
    ...
    $.hub.dispatcher.getManager('payment').getDecorator().loadUsblpBonusesSelector(checked);
  ...
  "#usbl_selector -> change": function(ev){ // otp dropdown
    $.hub.dispatcher.getManager('payment').getDecorator().setInterfaceForUsblpUse($('[name=use_usblp_bonuses]:enabled').prop("checked"));
  },
...
function AmexActivatedState()
{
    ...
    this.set_cvv_description = function(context)
    {
        var card = context.self,
            ...
            cvv_description = $.archive && $.archive.getData().translations && $.archive.getData().translations.cvv_description;
